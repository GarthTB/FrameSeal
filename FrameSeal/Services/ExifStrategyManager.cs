...
internal static class ExifStrategyManager
{
    ...
    public static string[] ExifKeys => [.. _strategies.Keys];

    /// <returns> 用于提取某项EXIF元数据的方法 </returns>
    public static ExifStrategy GetExifStrategy(string key, string manualText)
        => _strategies.TryGetValue(key, out var func)
        ? func ?? (_ => manualText)
        : throw new KeyNotFoundException($"EXIF 中不存在 {key} 项");

    /// <summary> 用于提取各项EXIF元数据的方法 </summary>
    private static readonly Dictionary<string, ExifStrategy?> _strategies = new()
    {
        ["手动输入"] = null,
        ["相机型号"] = (exif) => exif?.GetValue(ExifTag.Model)?.Value,
        ["相机厂商"] = (exif) => exif?.GetValue(ExifTag.Make)?.Value,
        ["镜头型号"] = (exif) => exif?.GetValue(ExifTag.LensModel)?.Value,
        ["镜头厂商"] = (exif) => exif?.GetValue(ExifTag.LensMake)?.Value,
        ["实际焦距"] = GetFocalLength,
        ...
        ["作者"] = (exif) => exif?.GetValue(ExifTag.Artist)?.Value,
        ["版权"] = (exif) => exif?.GetValue(ExifTag.Copyright)?.Value,
        ["软件"] = (exif) => exif?.GetValue(ExifTag.Software)?.Value
    };
